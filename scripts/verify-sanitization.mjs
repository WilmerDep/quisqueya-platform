...
const checks = [
  ...
  },
  {
    ...
    forbidden: ['services/dataService', 'localStorage', 'prestard_', 'getSaaSPlans', 'getPayments', 'getNodesTelemetry', 'saveSaaSPlan'],
    required: ['teamService.list()', 'organizationService.load()', 'auditService.list()'],
    reason: 'the active admin surface must use neutral API-backed organization boundaries only',
  },
  {
  ...
  {
    file: 'prisma/models/content.prisma',
    forbidden: ['Loan', 'loan', 'mora', 'collector', 'cashMovement', 'Payment'],
    required: ['model MediaAsset', 'model Experience', 'model Destination', 'model ContentPage', 'model TaxonomyTerm', 'model ExperienceDestination', 'provenanceJson', 'storageKey'],
  ...
  },
  {
    ...
    reason: 'Prisma must load the schema directory so CRM and content domain files remain isolated from the inherited monolithic schema',
  },
  {
